...
import { writeCanvasState } from '@/services/canvas-state';
import type {
  CanvasState,
  SectionState,
  DCContextValue,
  Transform,
} from '@/types/design-canvas';
...
// ─────────────────────────────────────────────────────────────
// dcFlatten — recursively unwrap React.Fragment so <>…</> grouping
// doesn't hide DCSection/DCArtboard children from the type-based walks below.
// ─────────────────────────────────────────────────────────────
function dcFlatten(children: React.ReactNode): React.ReactNode[] {
  const out: React.ReactNode[] = [];
  React.Children.forEach(children, (c) => {
    if (c && React.isValidElement(c) && c.type === React.Fragment) {
      out.push(...dcFlatten((c.props as { children?: React.ReactNode }).children));
    } else {
      out.push(c);
    }
  });
...
// ─────────────────────────────────────────────────────────────
// dcExport — per-artboard PNG / HTML export
// ─────────────────────────────────────────────────────────────
async function dcExport(
  node: HTMLElement,
  w: number,
  h: number,
  name: string,
  ...
  const cloneStyled = (src: Node): Node => {
    if (src.nodeType === 8) return document.createTextNode('');
    if (src.nodeType === 1 && (src as Element).tagName === 'SCRIPT') return document.createTextNode('');
    ...
    if (src.nodeType === 1) {
      ...
      dstEl.setAttribute('style', txt + 'animation:none;transition:none;');
    ...
    for (let c = src.firstChild; c; c = c.nextSibling) {
      (dst as Element).appendChild(cloneStyled(c));
    }
    return dst;
  };
    ...
    while ((m = re.exec(bg))) {
      ...
      jobs.push(
        toDataURL(url).then((d) => {
          htmlEl.style.backgroundImage = htmlEl.style.backgroundImage
            .split(tok)
            .join('url("' + d + '")');
        }),
      );
...
// ─────────────────────────────────────────────────────────────
// DCArtboard — marker component; rendered by DCArtboardFrame via DCSection.
// ─────────────────────────────────────────────────────────────
export function DCArtboard(_props: {
  id?: string;
  label?: string;
  width?: number;
  height?: number;
...
// ─────────────────────────────────────────────────────────────
// DCEditable — inline rename; commits on blur or Enter.
// ─────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────
// DCArtboardFrame — per-artboard wrapper with header, grip, label,
// kebab menu, and expand button.
// ─────────────────────────────────────────────────────────────
...
function DCViewport({ children, minScale = 0.1, maxScale = 8, style = {} }: DCViewportProps) {
  ...
  const tf = useRef<Transform>({ x: 0, y: 0, scale: 1 });
  // Persist viewport across reloads so the user lands back where they were
  // after an agent edit or browser refresh. The sandbox origin is already
  // per-project; pathname keeps multiple canvas files in one project apart.
  const tfKey = 'dc-viewport:' + location.pathname;
  const saveT = useRef<ReturnType<typeof setTimeout>>(0 as unknown as ReturnType<typeof setTimeout>);
  ...
  useLayoutEffect(() => {
    ...
    try {
      const s = JSON.parse(localStorage.getItem(tfKey) || 'null') as Transform | null;
  ...
  useEffect(() => {
    ...
    let panDrag: { id: number; lx: number; ly: number } | null = null;
    const onPointerDown = (e: PointerEvent) => {
      const onBg = !(e.target as Element).closest('[data-dc-slot], .dc-editable');
      if (!(e.button === 1 || (e.button === 0 && onBg))) return;
      e.preventDefault();
      vp.setPointerCapture(e.pointerId);
      panDrag = { id: e.pointerId, lx: e.clientX, ly: e.clientY };
      vp.style.cursor = 'grabbing';
    };
    const onPointerMove = (e: PointerEvent) => {
      if (!panDrag || e.pointerId !== panDrag.id) return;
      tf.current.x += e.clientX - panDrag.lx;
      tf.current.y += e.clientY - panDrag.ly;
      panDrag.lx = e.clientX;
      panDrag.ly = e.clientY;
      apply();
    };
    ...
    const onHostMsg = (e: MessageEvent) => {
      const d = e.data as { type?: string; scale?: number } | null;
...
export function DCSection({ id, title, subtitle, children, gap = 48 }: DCSectionProps) {
  ...
  const artboards = all.filter(
    (c): c is React.ReactElement => React.isValidElement(c) && c.type === DCArtboard,
  );
  const rest = all.filter((c) => !(React.isValidElement(c) && c.type === DCArtboard));
  const sec = (ctx && sid && ctx.section(sid)) || ({} as SectionState);
  // Must match DesignCanvas's srcKey computation exactly (it filters falsy
  // IDs), or onDelete persists a srcKey that DesignCanvas never recognizes.
  const allIds = artboards
    .map((a) => {
      const p = a.props as { id?: string; label?: string };
      return p.id ?? p.label;
    })
    ...
    >
      ...
      >
        {order.map((k) => {
          const ab = byId[k];
          if (!ab) return null;
          const abProps = ab.props as { id?: string; label?: string };
          return (
            <DCArtboardFrame
              key={k}
              sectionId={sid}
              artboard={ab}
              order={order}
              label={sec.labels?.[k] ?? abProps.label}
              onRename={(v) =>
                ctx &&
...
// ─────────────────────────────────────────────────────────────
// DCFocusOverlay — fullscreen focus mode with ←/→/↑/↓/Esc navigation
// ─────────────────────────────────────────────────────────────
interface DCFocusOverlayProps {
  entry: { sectionId: string; artboard: React.ReactElement };
  sectionMeta: Record<
    string,
    { title: string; subtitle?: string; slotIds: string[]; orderedIds: string[] }
  >;
  sectionOrder: string[];
}
...
function DCFocusOverlay({ entry, sectionMeta, sectionOrder }: DCFocusOverlayProps) {
  ...
  const artboardProps = artboard.props as {
    id?: string;
    label?: string;
    width?: number;
    height?: number;
  ...
  const aid = artboardProps.id ?? artboardProps.label ?? '';
  ...
  const { width = 260, height = 480, children } = artboardProps;
  ...
  const Arrow = ({ dir, onClick }: ArrowProps) => (
    <button
  ...
  return ReactDOM.createPortal(
    ...
    >
                ...
                .map((sid) => (
                  <button
                    ...
                    style={{
                      ...
                      cursor: 'pointer',
                      background:
                        sid === sectionId ? 'rgba(255,255,255,.1)' : 'transparent',
                      color: '#fff',
                      padding: '8px 12px',
                      borderRadius: 5,
                      fontSize: 14,
      ...
      >
        ...
        >
          {sec.labels?.[aid] ?? artboardProps.label}
...
// artboards, focused artboard). Order/titles/labels/hidden persist via
// writeCanvasState from @/services/canvas-state. Focus is ephemeral.
// ─────────────────────────────────────────────────────────────
...
export function DesignCanvas({ children, minScale, maxScale, style }: DesignCanvasProps) {
  const [state, setState] = useState<CanvasState>({ sections: {}, focus: null });
  // ready becomes true immediately after first render — no sidecar read to wait for.
  ...
  // Persist section state on every change.
  useEffect(() => {
    const t = setTimeout(() => {
      writeCanvasState(state.sections);
    }, 250);
  ...
  const registry: Record<string, { sectionId: string; artboard: React.ReactElement }> = {};
  ...
  dcFlatten(children).forEach((sec) => {
    ...
    const secProps = sec.props as DCSectionProps;
    const sid = secProps.id ?? secProps.title;
    if (!sid) return;
    sectionOrder.push(sid);
    const persisted = state.sections[sid] || ({} as SectionState);
    const abs: Array<[string, React.ReactElement]> = [];
    dcFlatten(secProps.children).forEach((ab) => {
    ...
    sectionMeta[sid] = {
      ...
      orderedIds: [...kept, ...srcIds.filter((k) => !kept.includes(k))],
    };
  });

  const api = useMemo<DCContextValue>(
    () => ({
      state,
      section: (id: string) => state.sections[id] || ({} as SectionState),
      patchSection: (
        id: string,
        p: Partial<SectionState> | ((prev: SectionState) => SectionState),
      ) =>
        setState((s) => ({
          ...s,
          sections: {
            ...
            [id]: {
              ...s.sections[id],
              ...(typeof p === 'function' ? p(s.sections[id] || ({} as SectionState)) : p),
            },
          },
        })),
